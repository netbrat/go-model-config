...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  var comTpl = function(tpl, anim, back){
    return ['<div class="layim-panel'+ (anim ? ' layui-m-anim-left' : '') +'">'
      ,'<div class="layim-title" style="background-color: {{d.common.chatTitleColor}};">'
        ,'<p>'
          ,(back ? '<i class="layui-icon layim-chat-back" layim-event="back">&#xe603;</i>' : '') 
          ,'{{ d.title || d.common.title }}<span class="layim-chat-status"></span>'
          ,'{{# if(d.data){ }}'
  ...
  var elemTpl = ['<div class="layui-layim">'
    ...
    ,'<div class="layim-tab-content">'
      ,'<ul class="layim-list-top">'
        ,'{{# layui.each(d.base_view.py.moreList, function(index, item){ }}'
  ...
  var anim = 'layui-anim-upbit', events = { 
    ...
    chat: function(othis){
      var local = layui.data('layim-mobile')[cache.mine.id] || {};
      var type = othis.data('type'), index = othis.data('index_view.py');
      var list = othis.attr('data-list') || othis.index_view(), data = {};
      if(type === 'friend'){
    ...
    ,spread: function(othis){
      ...
      local['spread' + othis.parent().index_view()] = spread;
      layui.data('layim-mobile', {
        key: cache.mine.id
        ,value: local
      });
    ...
    //底部导航切换
    ,tab: function(othis){
      var index = othis.index_view(), main = '.layim-tab-content';
    ...
    ,back: function(othis){
      ...
      ,index = layero.attr('index_view.py')
      ,PANEL = '.layim-panel';
      setTimeout(function(){
        layer.close(index);
      }, 300);
    ...
    ,face: function(othis, e){
      ...
      layer.close(layer.popBottom.index_view);
