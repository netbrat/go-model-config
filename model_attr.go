...
type ModelBaseField struct {
   ...
   NotNullOption bool        `json:"not_null_option"`       //下拉型不显示显示“请选择"项 默认false
   ...
   EditHide      bool        `json:"edit_hide"`             //是否不显示在编辑表单中（false)
...
type ModelField struct {
   ...
   NotFilter bool   `json:"not_filter"`           //数据保存时是否对内容进行安全过虑(默认false)
   ...
   NotSort   bool   `json:"not_sort" `            //列表时是否允许排序（默认false)
}
...
type ModelListFieldIndex struct{
   Index int
   Name string
}
...
type ModelAttr struct {
   ...
   NotAutoInc     bool                         `json:"not_auto_inc"`                   //主键是否自增长		默认 false
   ...
   //listFieldIndex []ModelListFieldIndex        `json:"-"`                              //字段索引
}
...
func (attr *ModelAttr) parse() {
   ...
   // 分析列表字段的基础字段信息
   //attr.listFieldIndex = make([]ModelListFieldIndex,0)
   for i, _ := range attr.Fields {
       f := &attr.Fields[i]
       //if !attr.Fields[i].Hidden {
         // attr.listFieldIndex = append(attr.listFieldIndex, ModelListFieldIndex{Index:i, Name:f.Name})
       //}
       if f.Name == attr.Pk { //当字段是主键时，判断是否可编辑及必填属性
          f.Disabled = !attr.NotAutoInc
          f.Required = attr.NotAutoInc
       }
